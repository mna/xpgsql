...
local function exec_or_query(rawconn, stmt, success, ...)
  ...
  if not res then
    return nil, rawconn:errorMessage()
  end
...
-- Executes a query statement and returns the result if it succeeds, or
-- nil and an error message. Note that INSERT .. RETURNING must use
-- Connection:query as it returns values. The statement may contain $1,
-- $2, etc.  placeholders, they will be replaced by the extra arguments
